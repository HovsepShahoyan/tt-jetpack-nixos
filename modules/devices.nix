...
let
  ...
  nvpModelConf = {
    ...
    dualorin-agx = "${pkgs.nvidia-jetpack.l4t-nvpmodel}/etc/nvpmodel/nvpmodel_p3701_0000.conf";
  ...
  nvfancontrolConf = {
    ...
    dualorin-agx = "${pkgs.nvidia-jetpack.l4t-nvfancontrol}/etc/nvpower/nvfancontrol/nvfancontrol_p3701_0000.conf";
...
lib.mkMerge [{
  ...
  hardware.nvidia-jetpack.flashScriptOverrides =
    ...
    mkMerge [
      ...
      (mkIf (cfg.som == "dualorin-agx") {
        targetBoard = mkDefault "jetson-agx-dualorin-devkit";
        # We don't flash the sdmmc with kernel/initrd/etc at all. Just let it be a
        # regular NixOS machine instead of having some weird partition structure.
        partitionTemplate = mkDefault "${pkgs.nvidia-jetpack.bspSrc}/bootloader/t186ref/cfg/flash_t234_qspi.xml";
      })
